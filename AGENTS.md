...
- **Delegation Router V1 boundary**: Delegation is disabled by default and exposes only execution-scoped `bulk_read`. Hooks are fail-open cost guardrails, not security enforcement; source reaches a worker through its prompt, and worker eligibility requires a proven tool-less/scratch-only provider contract. Unsupported providers and `raw-shell` are skipped. Workers run one process per request at depth 1, receive no repository tools/MCP, and never make implementation or architecture decisions. Every `process_pid > 0` is ownership regardless of status; PID/identity must be persisted before reservation release, unresolved Stop retains `recovery_required`, and only confirmed dead/mismatch/termination releases capacity. Passive recovery is single-flight. Managed hook verification is definition/install-specific and checks the copied bridge SHA-256. Never suppress unrelated primary-process MCP configuration, persist source/prompt/raw shell text, or invent token savings.
