...
- **Delegation Router V1 boundary**: Delegation is disabled by default and exposes only execution-scoped `bulk_read`. Hooks are fail-open cost guardrails, not security enforcement; workers run one Claude/Codex/Antigravity process per request at depth 1, receive no repository tools/MCP, and never make implementation or architecture decisions. Worker PID/identity ownership must be persisted before reservation release; unresolved Stop retains `recovery_required`, and only confirmed dead/mismatch/termination releases capacity. Managed hook verification is definition-and-installation-specific. Never admit `raw-shell`, suppress unrelated primary-process MCP configuration, persist source/prompt/raw shell text, or invent token savings.
