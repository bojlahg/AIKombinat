...
import { LayoutDashboard, Moon, Sun, Bell, BellOff, LogOut, Plus, X, Inbox, CalendarDays, Terminal, FileCode, Link as LinkIcon, Edit2, Settings, Cloud, PanelLeftClose, PanelLeftOpen, FlaskConical } from 'lucide-react';
    ...
    return (
      <div className="flex flex-col items-center h-full glass border-none py-3">
        ...
        <Link
          ...
          onClick={handleNav}
          className="relative flex items-center justify-center w-9 h-9 rounded-lg transition-colors hover:bg-theme-hover mt-0.5"
          style={location.pathname.startsWith('/experiments/agent-forum')
            ? { backgroundColor: 'var(--color-bg-hover)', color: 'var(--color-text-primary)' }
            : { color: 'var(--color-text-tertiary)' }}
          title={t('sidebar.agentForum')}
        >
      ...
      <div className="px-3 pt-3">
        ...
        <nav className="space-y-0.5">
          <Link
            to="/experiments/agent-forum"
            onClick={handleNav}
            className={`relative flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all duration-200 active:scale-95 ${location.pathname.startsWith('/experiments/agent-forum') ? 'font-medium' : ''}`}
            style={location.pathname.startsWith('/experiments/agent-forum')
              ? { backgroundColor: 'var(--color-bg-hover)', color: 'var(--color-text-primary)', boxShadow: 'var(--shadow-soft)' }
              : { color: 'var(--color-text-tertiary)' }
            }
          >
            {location.pathname.startsWith('/experiments/agent-forum') && (
              <span className="absolute left-0 top-1/4 bottom-1/4 w-[3px] rounded-r-full" style={{ backgroundColor: 'var(--color-accent)' }} />
            )}
            <FlaskConical size={16} />
            {t('sidebar.agentForum')}
          </Link>
        </nav>
      </div>
