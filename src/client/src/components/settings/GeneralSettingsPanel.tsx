    ...
    <div className="p-6 sm:p-8">
      ...
      {desktop?.supported && (
        <section className="mt-8 border-t pt-7" style={{ borderColor: 'var(--color-border)' }}>
          ...
          <label className={`flex items-start gap-3 rounded-xl p-3 ${desktop.packaged ? 'cursor-pointer hover:bg-theme-hover' : 'opacity-60'}`}>
            <input
              type="checkbox"
              checked={desktop.openAtLogin}
              disabled={!desktop.packaged || desktopBusy}
            ...
            <span>
              ...
              {!desktop.packaged && <span className="mt-1 block text-xs text-warm-400">{t('settings.desktop.packagedOnly')}</span>}
            </span>
          </label>
          ...
          <fieldset className="mt-5" disabled={desktopBusy}>
            ...
            <label className="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 hover:bg-theme-hover">
              <input type="radio" name="closeBehavior" checked={desktop.closeBehavior === 'tray'} onChange={() => updateDesktop({ closeBehavior: 'tray' })} />
              <span className="text-sm text-warm-600">{t('settings.desktop.closeToTray')}</span>
            </label>
            <label className="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 hover:bg-theme-hover">
