...
describe('startup process recovery', () => {
  ...
  it.each(['mismatch', 'unverifiable'] as const)('fails closed for a live %s identity without signalling or releasing ownership', async (verdict) => {
    const owners = runningOwners();
    resourceManager.acquireAtomic({ ownerType: 'todo', ownerId: owners.todo.id, runToken: `todo-${verdict}`, resources: ['gpu.0'] });
    await recoverPersistedProcesses({ isAlive: () => true, verify: vi.fn().mockResolvedValue(verdict) });
